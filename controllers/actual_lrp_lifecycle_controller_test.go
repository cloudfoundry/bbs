...
var _ = FDescribe("ActualLRP Lifecycle Controller", func() {
	Describe("Unit tests for individual Actual LRP actions", func() {
		var (
		...
		Describe("StartActualLRP", func() {
			...
			Context("when there is a Suspect LRP running", func() {
				...
				It("emits ActualLRPCreatedEvent and an ActualLRPRemovedEvent", func() {
					...
					e = actualHub.EmitArgsForCall(0).(*models.ActualLRPCreatedEvent)
					Expect(e.ActualLrpGroup).To(Equal(afterActualLRP.ToActualLRPGroup()))

					var e2 *models.ActualLRPRemovedEvent
					Expect(actualHub.EmitArgsForCall(1)).To(BeAssignableToTypeOf(e2))
					e2 = actualHub.EmitArgsForCall(1).(*models.ActualLRPRemovedEvent)
					Expect(e2.ActualLrpGroup).To(Equal(suspect.ToActualLRPGroup()))
				})
		...
		Describe("CrashActualLRP", func() {
			...
			It("emits crashed and changed event to the actual instance hub", func() {
				...
				Expect(events).To(ConsistOf(&models.ActualLRPCrashedEvent{
					...
					&models.ActualLRPInstanceChangedEvent{
						Before: actualLRP,
			...
			Context("when the crashed instance is a replacement for a Suspect ActualLRP", func() {
				...
				It("should not emit an ActualLRPChangedEvent because the ActualLRPGroup remains unchanged", func() {
					err = controller.CrashActualLRP(logger, &actualLRPKey, &beforeInstanceKey, errorMessage)
					Expect(err).NotTo(HaveOccurred())
			...
			Context("when the LRP being crashed is a Suspect LRP", func() {
				...
				Context("and a replacement instance has already been created, but not claimed", func() {
					...
					It("emits an ActualLRPInstanceCreatedEvent and an ActualLRPInstanceRemovedEvent because the replacement instance has taken over", func() {
						err = controller.CrashActualLRP(logger, &actualLRPKey, &beforeInstanceKey, errorMessage)
						Eventually(actualLRPInstanceHub.EmitCallCount).Should(Equal(2))
						...
						Expect(event).To(BeAssignableToTypeOf(createdEvent))
						createdEvent = event.(*models.ActualLRPInstanceCreatedEvent)
						Expect(createdEvent.ActualLrp).To(Equal(replacementLRP))
				...
				Context("and a replacement instance has already been claimed", func() {
					...
					It("emits an ActualLRPInstanceCreatedEvent and an ActualLRPInstanceRemovedEvent because the replacement instance has taken over the ActualLRP", func() {
						err = controller.CrashActualLRP(logger, &actualLRPKey, &beforeInstanceKey, errorMessage)
						Eventually(actualLRPInstanceHub.EmitCallCount).Should(Equal(2))
	...
	Describe("Actual LRP Lifecycles", func() {
		Context("When there is a CLAIMED SUSPECT instance and an UNCLAIMED ORDINARY instance to replace it", func() {
			var (
				controller *controllers.ActualLRPLifecycleController
				logger     *lagertest.TestLogger
				...
				index                            int32
				actualLRPKey                     models.ActualLRPKey
				originalLRPInstanceKey           models.ActualLRPInstanceKey
				claimedReplacementLRPInstanceKey models.ActualLRPInstanceKey

				originalSuspectLRP      models.ActualLRP
				unclaimedReplacementLRP models.ActualLRP
				claimedReplacementLRP   models.ActualLRP
				runningReplacementLRP   models.ActualLRP
			...
			BeforeEach(func() {
				...
				originalLRPInstanceKey = models.NewActualLRPInstanceKey(
					"instance-guid-0",
					"cell-id-0",
				)
				...
				originalSuspectLRP = models.ActualLRP{
					ActualLRPKey:         actualLRPKey,
					ActualLRPInstanceKey: originalLRPInstanceKey,
					State:                models.ActualLRPStateClaimed,
					Since:                1138,
					Presence:             models.ActualLRP_Suspect,
				}
				...
				claimedReplacementLRPInstanceKey = models.NewActualLRPInstanceKey(
					"instance-guid-0",
					"cell-id-1",
				)
				claimedReplacementLRP = unclaimedReplacementLRP
				claimedReplacementLRP.ActualLRPInstanceKey = claimedReplacementLRPInstanceKey
				claimedReplacementLRP.State = models.ActualLRPStateClaimed

				runningReplacementLRP = claimedReplacementLRP
				runningReplacementLRP.State = models.ActualLRPStateRunning

				fakeActualLRPDB.ClaimActualLRPReturnsOnCall(0, &unclaimedReplacementLRP, &claimedReplacementLRP, nil)
				fakeActualLRPDB.ActualLRPsReturnsOnCall(0, []*models.ActualLRP{&originalSuspectLRP, &claimedReplacementLRP}, nil)
				fakeSuspectDB.RemoveSuspectActualLRPReturns(&originalSuspectLRP, nil)
			})

			Specify("how to replace a CLAIMED instance on a suspect cell", func() {
				By("Another cell claims the replacement instance")
				err := controller.ClaimActualLRP(logger, processGuid, index, &claimedReplacementLRPInstanceKey)
				Expect(err).NotTo(HaveOccurred())

				Expect(fakeActualLRPDB.ClaimActualLRPCallCount()).To(Equal(1))
				_, requestedProcessGuid, requestedIndex, requestedActualLRPInstanceKey := fakeActualLRPDB.ClaimActualLRPArgsForCall(0)
				Expect(requestedProcessGuid).To(Equal(processGuid))
				Expect(requestedIndex).To(Equal(index))
				Expect(*requestedActualLRPInstanceKey).To(Equal(claimedReplacementLRPInstanceKey))

				By("The suspect cell tries to CLAIM its instance again and the BBS rejects it")
				fakeActualLRPDB.ClaimActualLRPReturnsOnCall(1, nil, nil, models.ErrActualLRPCannotBeClaimed)

				err = controller.ClaimActualLRP(logger, processGuid, index, &originalLRPInstanceKey)
				Expect(err).To(HaveOccurred())

				_, requestedProcessGuid, requestedIndex, requestedActualLRPInstanceKey = fakeActualLRPDB.ClaimActualLRPArgsForCall(1)
				Expect(requestedProcessGuid).To(Equal(processGuid))
				Expect(requestedIndex).To(Equal(index))
				Expect(*requestedActualLRPInstanceKey).To(Equal(originalLRPInstanceKey))

				By("the other cell starts the replacement instance")
				fakeActualLRPDB.ActualLRPsReturnsOnCall(1, []*models.ActualLRP{&originalSuspectLRP, &claimedReplacementLRP}, nil)
				fakeActualLRPDB.StartActualLRPReturnsOnCall(0, &claimedReplacementLRP, &runningReplacementLRP, nil)

				netInfo := models.NewActualLRPNetInfo("1.1.1.1", "2.2.2.2", models.NewPortMapping(10, 20))
				err = controller.StartActualLRP(logger, &actualLRPKey, &claimedReplacementLRPInstanceKey, &netInfo)
				Expect(err).NotTo(HaveOccurred())

				By("the BBS removes the suspect ActualLRP")
				Expect(fakeSuspectDB.RemoveSuspectActualLRPCallCount()).To(Equal(1))
				_, requestedActualLRPKey := fakeSuspectDB.RemoveSuspectActualLRPArgsForCall(0)
				Expect(*requestedActualLRPKey).To(Equal(actualLRPKey))

				By("Asserting that the BBS API shows that the ALG's instance is the RUNNING ORDINARY replacement instance")
				Eventually(actualHub.EmitCallCount).Should(Equal(4))
				Consistently(actualHub.EmitCallCount).Should(Equal(4))

				events := []models.Event{
					actualHub.EmitArgsForCall(0),
					actualHub.EmitArgsForCall(1),
					actualHub.EmitArgsForCall(2),
					actualHub.EmitArgsForCall(3),
				}
				Expect(events).To(ContainElement(&models.ActualLRPCreatedEvent{ActualLrpGroup: claimedReplacementLRP.ToActualLRPGroup()}))
				// Expect(events).To(ContainElement(&models.ActualLRPChangedEvent{ActualLrpGroup: claimedReplacementLRP.ToActualLRPGroup()}))
				Expect(events).To(ContainElement(&models.ActualLRPCreatedEvent{ActualLrpGroup: claimedReplacementLRP.ToActualLRPGroup()}))
				Expect(events).To(ContainElement(&models.ActualLRPCreatedEvent{ActualLrpGroup: claimedReplacementLRP.ToActualLRPGroup()}))

				// Expect exactly one CreatedEvent to be emitted
				// Expect exactly one ChangedEvent to be emitted for claiming the replacement instance -- should this one exist???
				// Expect exactly one ChangedEvent to be emitted for starting the replacement instance
				// Expect exactly one RemovedEvent to be emitted for removing the original suspect instance
			})
		})
	})
})
