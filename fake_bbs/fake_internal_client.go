...
type FakeInternalClient struct {
	...
	SubscribeToEventsForCellStub        func(logger lager.Logger, cellID string) (events.EventSource, error)
	subscribeToEventsForCellMutex       sync.RWMutex
	subscribeToEventsForCellArgsForCall []struct {
		logger lager.Logger
		cellID string
	}
	subscribeToEventsForCellReturns struct {
		result1 events.EventSource
		result2 error
	}
...
func (fake *FakeInternalClient) SubscribeToEventsForCell(logger lager.Logger, cellID string) (events.EventSource, error) {
	fake.subscribeToEventsForCellMutex.Lock()
	fake.subscribeToEventsForCellArgsForCall = append(fake.subscribeToEventsForCellArgsForCall, struct {
		logger lager.Logger
		cellID string
	}{logger, cellID})
	fake.subscribeToEventsForCellMutex.Unlock()
	if fake.SubscribeToEventsForCellStub != nil {
		return fake.SubscribeToEventsForCellStub(logger, cellID)
	} else {
		return fake.subscribeToEventsForCellReturns.result1, fake.subscribeToEventsForCellReturns.result2
	}
}

func (fake *FakeInternalClient) SubscribeToEventsForCellCallCount() int {
	fake.subscribeToEventsForCellMutex.RLock()
	defer fake.subscribeToEventsForCellMutex.RUnlock()
	return len(fake.subscribeToEventsForCellArgsForCall)
}

func (fake *FakeInternalClient) SubscribeToEventsForCellArgsForCall(i int) (lager.Logger, string) {
	fake.subscribeToEventsForCellMutex.RLock()
	defer fake.subscribeToEventsForCellMutex.RUnlock()
	return fake.subscribeToEventsForCellArgsForCall[i].logger, fake.subscribeToEventsForCellArgsForCall[i].cellID
}

func (fake *FakeInternalClient) SubscribeToEventsForCellReturns(result1 events.EventSource, result2 error) {
	fake.SubscribeToEventsForCellStub = nil
	fake.subscribeToEventsForCellReturns = struct {
		result1 events.EventSource
		result2 error
	}{result1, result2}
}
