...
func (db *SQLDB) CountActualLRPsForProcessGuidAndState(ctx context.Context, logger lager.Logger, processGuid string, state string, presence models.ActualLRP_Presence) (int, error) {
	query := `
	...
	row := db.db.QueryRowContext(ctx, db.helper.Rebind(query), processGuid, state, presence)
