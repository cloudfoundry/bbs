...
var _ = Describe("DesiredLRPDB", func() {
	...
	Describe("DesiredLRPByProcessGuid", func() {
		...
		Context("when the run info is invalid", func() {
			BeforeEach(func() {
				...
				queryStr := `UPDATE lrp_deployments SET run_info = ? WHERE definition_guid = ?`
		...
		Context("when the routes are invalid", func() {
			BeforeEach(func() {
				queryStr := `UPDATE lrp_deployments SET routes = ? WHERE definition_guid = ?`
				if test_helpers.UsePostgres() {
					queryStr = test_helpers.ReplaceQuestionMarks(queryStr)
				}
				result, err := db.Exec(queryStr, "{{", expectedDesiredLRP.ProcessGuid)
	...
	Describe("DesiredLRPs", func() {
		...
		It("prunes all desired lrps with invalid run infos", func() {
			...
			queryStr := `UPDATE lrp_deployments SET run_info = 'garbage' WHERE definition_guid = 'invalid'`
			if test_helpers.UsePostgres() {
				queryStr = test_helpers.ReplaceQuestionMarks(queryStr)
			}
			...
			rows, err := db.Query(`SELECT process_guid FROM lrp_deployments`)
			Expect(err).NotTo(HaveOccurred())
			defer rows.Close()
			processGuids := []string{}
			for rows.Next() {
