...
func (db *SQLDB) DesireLRP(logger lager.Logger, desiredLRP *models.DesiredLRP) error {
	...
	return db.transact(logger, func(logger lager.Logger, tx helpers.Tx) error {
		...
		runInfoData, err := internal.SerializeModel(logger, db.serializer, &runInfo)
		if err != nil {
		...
		volumePlacementData, err := internal.SerializeModel(logger, db.serializer, volumePlacement)
		if err != nil {
			logger.Error("failed-to-serialize-model", err)
			return err
		}
		...
		_, err = db.helper.Insert(logger, tx, desiredLRPsTable,
...
func (db *SQLDB) DesiredLRPByProcessGuid(logger lager.Logger, processGuid string) (*models.DesiredLRP, error) {
	...
	err := db.transact(logger, func(logger lager.Logger, tx helpers.Tx) error {
		var err error
		row := db.helper.One(logger, tx, desiredLRPsTable,
...
func (db *SQLDB) DesiredLRPs(logger lager.Logger, filter models.DesiredLRPFilter) ([]*models.DesiredLRP, error) {
	...
	err := db.transact(logger, func(logger lager.Logger, tx helpers.Tx) error {
		rows, err := db.helper.All(logger, tx, desiredLRPsTable,
			desiredLRPColumns, helpers.NoLockRow,
...
func (db *SQLDB) UpdateDesiredLRP(logger lager.Logger, processGuid string, update *models.DesiredLRPUpdate) (*models.DesiredLRP, error) {
	...
	err := db.transact(logger, func(logger lager.Logger, tx helpers.Tx) error {
		...
		_, err = db.helper.Update(logger, tx, desiredLRPsTable, updateAttributes, `process_guid = ?`, processGuid)
		if err != nil {
...
func (db *SQLDB) RemoveDesiredLRP(logger lager.Logger, processGuid string) error {
	...
	return db.transact(logger, func(logger lager.Logger, tx helpers.Tx) error {
		...
		_, err = db.helper.Delete(logger, tx, desiredLRPsTable, "process_guid = ?", processGuid)
		if err != nil {
...
func (db *SQLDB) fetchDesiredLRPSchedulingInfoAndMore(logger lager.Logger, scanner helpers.RowScanner, dest ...interface{}) (*models.DesiredLRPSchedulingInfo, error) {
	...
	err = internal.DeserializeModel(logger, db.serializer, volumePlacementData, &volumePlacement)
	if err != nil {
...
func (db *SQLDB) fetchDesiredLRPInternal(logger lager.Logger, scanner helpers.RowScanner) (*models.DesiredLRP, string, error) {
	...
	err = internal.DeserializeModel(logger, db.serializer, runInfoData, &runInfo)
	if err != nil {
...
func (db *SQLDB) deleteInvalidLRPs(logger lager.Logger, queryable helpers.Queryable, guids ...string) error {
	for _, guid := range guids {
		...
		_, err := db.helper.Delete(logger, queryable, desiredLRPsTable, "process_guid = ?", guid)
		if err != nil {
