...
func (h *EvacuationHandler) EvacuateClaimedActualLRP(logger lager.Logger, w http.ResponseWriter, req *http.Request) {
	...
	keepRouteForSuspect := false
	if beforeActualLRPGroup.Suspect != nil && beforeActualLRPGroup.Instance.ActualLRPInstanceKey.InstanceGuid == request.ActualLrpInstanceKey.InstanceGuid {
		keepRouteForSuspect = true
	}
	...
	if err == nil {
		...
		if err != nil {
			...
			if !keepRouteForSuspect {
				go h.actualHub.Emit(models.NewActualLRPRemovedEvent(beforeActualLRPGroup))
			}
		}
	}

	err = h.unclaimAndRequestAuction(logger, request.ActualLrpKey, keepRouteForSuspect)
...
func (h *EvacuationHandler) EvacuateRunningActualLRP(logger lager.Logger, w http.ResponseWriter, req *http.Request) {
	...
	suspect := actualLRPGroup.Suspect
	...
	if (instance.State == models.ActualLRPStateClaimed || instance.State == models.ActualLRPStateRunning) &&
		...
		err = h.unclaimAndRequestAuction(logger, request.ActualLrpKey, false)
		if err != nil {
			response.Error = models.ConvertError(err)
			return
		}
...
func (h *EvacuationHandler) unclaimAndRequestAuction(logger lager.Logger, lrpKey *models.ActualLRPKey, keepRouteForSuspect bool) error {
