...
var _ = Describe("Event Handlers", func() {
	...
	FDescribe("Subscribe_r0", func() {
		...
		BeforeEach(func() {
			...
			handler = handlers.NewLRPInstanceEventHandler(lrpInstanceHub)
		})
		...
		Describe("Subscribe to Instance Events", func() {
			...
			Context("when cell id is specified", func() {
				...
				Context("ActualLRPInstanceChangedEvent", func() {
					...
					FContext("subscriber with the right filter", func() {
						BeforeEach(func() {
							requestBody = &models.EventsByCellId{
								CellId: cellId,
							}
						...
						Context("and an LRP transitions to evacuating", func() {
							...
							FIt("receives changed events if the lrp is running on the cell", func() {
								Eventually(eventsCh).Should(Receive(Equal(expectedActualLRPBeforeEvent)))
							})
						})
