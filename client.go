...
func (c *client) ActualLRPs(logger lager.Logger, filter models.ActualLRPFilter) ([]*models.ActualLRP, error) {
	...
	err := c.doRequest(logger, ActualLRPsRoute, nil, nil, &request, &response)
	if err != nil {
		return nil, err
	}
...
func (c *client) ActualLRPsByProcessGuid(logger lager.Logger, processGuid string) ([]*models.ActualLRP, error) {
	...
	response := models.ActualLRPsResponse{}
	err := c.doRequest(logger, ActualLRPsByProcessGuidRoute, nil, nil, &request, &response)
	if err != nil {
		return nil, err
	}
...
func (c *client) ActualLRPByProcessGuidAndIndex(logger lager.Logger, processGuid string, index int) (*models.ActualLRP, error) {
	...
	err := c.doRequest(logger, ActualLRPByProcessGuidAndIndexRoute, nil, nil, &request, &response)
	if err != nil {
		return nil, err
	}

	return response.ActualLrp, response.Error.ToError()
}
