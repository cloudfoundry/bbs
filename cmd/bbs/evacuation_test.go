...
var _ = Describe("Evacuation API", func() {
	...
	Describe("RemoveEvacuatingActualLRP", func() {
		Context("when the lrp is running", func() {
			BeforeEach(func() {
				Expect(client.StartActualLRP(logger, &actual.ActualLRPKey, &actual.ActualLRPInstanceKey, &actual.ActualLRPNetInfo, []*models.ActualLRPInternalRoute{}, map[string]string{})).To(Succeed())
			})

			It("removes the evacuating actual_lrp", func() {
				keepContainer, err := client.EvacuateRunningActualLRP(logger, &actual.ActualLRPKey, &actual.ActualLRPInstanceKey, &actual.ActualLRPNetInfo, []*models.ActualLRPInternalRoute{}, map[string]string{})
	...
	Describe("EvacuateRunningActualLRP", func() {
		BeforeEach(func() {
			err := client.StartActualLRP(logger, &actual.ActualLRPKey, &actual.ActualLRPInstanceKey, &actual.ActualLRPNetInfo, []*models.ActualLRPInternalRoute{}, map[string]string{})
			Expect(err).NotTo(HaveOccurred())
		})
