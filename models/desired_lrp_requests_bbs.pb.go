...
type DesiredLRPsRequest struct {
	...
	AppGuids     []string `json:"app_guids,omitempty"`
}
...
func (this *DesiredLRPsRequest) Equal(that interface{}) bool {
	...
	if this.AppGuids == nil {
		if that1.AppGuids != nil {
			return false
		}
	} else if len(this.AppGuids) != len(that1.AppGuids) {
		return false
	}
...
func (m *DesiredLRPsRequest) SetAppGuids(value []string) {
	if m != nil {
		m.AppGuids = value
	}
}
